...
const index = async (req, res) => {
    try {
        ...
        const result = await User_1.User.paginate(filter, options);
...
//@access   Private
exports.store = (0, express_async_handler_1.default)(async (req, res) => {
    const { avatar, name, email, gender, password } = req.body;
    const userExists = await User_1.User.findByEmail(email);
    if (userExists) {
